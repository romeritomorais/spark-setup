...
Opções:
     0)  spark-3.1.2-bin-hadoop3.2.tgz
...
if [ $DISTRO = fedora ] || [ $DISTRO = centos ]; then
    ...
    sudo pip3 uninstall spylon-kernel
    sudo pip3 uninstall pyspark
    sudo pip3 uninstall jupyterlab
    sudo yum update -y
...
if [ $SPARK = spark-3.1.2 ]; then
     SPARK_VERSION="Spark 3.1.2"
elif [ $SPARK = spark-3.0.1 ]; then
...
alias sparksql="/usr/local/spark/bin/spark-sql"
alias pyspark="/usr/local/spark/bin/pyspark"
source ~/.bashrc
